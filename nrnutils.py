...
def stringify(cls):
    """
     ...
     Note that 'getattr' (with 'value' as default) is used instead of just 'value' as inherited
...
class Section(nrn.Section):
    """
    Examples:
    >>> soma = Section(L=30, diam=30, mechanisms=[hh, leak], name='soma')
    >>> apical = Section(L=600, diam=2, nseg=5, mechanisms=[leak],
    ...                  parent=soma, connection_point=DISTAL)
    """

    __strprops__ = ['diam', 'cm', 'mechanisms', 'synapses', 'parent']

    def __init__(self, L, diam, nseg=1, Ra=100, cm=1, mechanisms=[], parent=None, connection_point=DISTAL, **kwargs):
        nrn.Section.__init__(self, **kwargs)
        # set geometry
        self.L = L
        self.diam = diam
        self.nseg = nseg
        ...
        if parent:
            ...
            self.parent = '%s(%f)' % (parent.name(), connection_point)
    ...
    def add_synapses(self, label, type, locations=[0.5], **parameters):
        ...
        self.synapses.append("%s(label=%s, locations=%s, parameters=%s)" % (type, label, locations, parameters))
...
if __name__ == "__main__":
    ...
    class SimpleNeuron(object):
        ...
        def __init__(self):
            ...
            self.soma = Section(name='soma', cm=2, L=30, diam=30, mechanisms=[leak,hh])
            ...
            self.soma.add_synapses('syn2', 'AlphaSynapse', locations=[0.1, 0.5], onset=5.0, gmax=0.04, e=-65)

        gnabar = uniform_property(["soma", "axon"], "hh.gnabar")
        gkbar = uniform_property(["soma", "axon"], "hh.gkbar")


    simple_neuron = SimpleNeuron()
    print(str(simple_neuron))
    print(simple_neuron.soma)
    print(simple_neuron.soma.mechanisms[0])
